...
template <int BM_T, int BN_T, int BK_T>
__device__ __forceinline__ void issue_tma_loads_for_tile(
    const CUtensorMap* tma_A_desc,
    const CUtensorMap* tma_B_desc,
    uint64_t* full_barrier,
    nv_bfloat16* sA_stage,
    nv_bfloat16* sBT_stage,
    const int block_offset_m,
    const int block_offset_n,
    const int k_start,
    const int M,
    const int N,
    const int K
) {
    constexpr int a_storage_elems = BM_T * BK_T;
    constexpr int b_storage_elems = BN_T * BK_T;

    // TMA skips OOB writes. Clear the destination first whenever a tile may
    // be partial so WGMMA never consumes stale shared-memory data.
    const bool partial_a_tile =
        block_offset_m + BM_T > M || k_start + BK_T > K;
    const bool partial_b_tile =
        block_offset_n + BN_T > N || k_start + BK_T > K;

    if (partial_a_tile) {
        for (int i = 0; i < a_storage_elems; ++i) {
            sA_stage[i] = __float2bfloat16(0.0f);
        }
    }
    if (partial_b_tile) {
        for (int i = 0; i < b_storage_elems; ++i) {
            sBT_stage[i] = __float2bfloat16(0.0f);
        }
    }
    ...
    constexpr uint32_t tx_count =
        sizeof(nv_bfloat16) * (a_storage_elems + b_storage_elems);
    mbarrier_arrive_expect_tx(full_barrier, tx_count);

    tma_load_2d(tma_A_desc, full_barrier, sA_stage, k_start, block_offset_m);
    tma_load_2d(tma_B_desc, full_barrier, sBT_stage, k_start, block_offset_n);
}
    ...
    if (is_tma_thread) {
        ...
        if (producer_tid == 0) {
            ...
            for (int kt = 0; kt < num_k_tiles; ++kt) {
                ...
                issue_tma_loads_for_tile<BM, BN, BK>(
                    &tma_A_desc,
                    &tma_B_desc,
                    &full_barrier[state.stage],
                    sA[state.stage],
                    sBT[state.stage],
                    block_offset_m,
                    block_offset_n,
...
extern "C" void ENTRYPOINT(
    ...
    // Hopper TMA swizzles operate on 16B chunks. The WGMMA descriptors below
    // use the matching K-major swizzle for the compile-time BK row width.
    ...
    res = cuTensorMapEncodeTiled(
        ...
        tma_swizzle_for_bf16_k(BK),         // swizzle
